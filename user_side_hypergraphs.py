...
def subgraph_key_building(subgraphs_mapping_i, users_list, num_items):
    ...
    if not 0 in subgraphs_mapping_i: #加上t=0后，非常大；t=0,u=,i
        subgraphs_mapping_i[0] = {}
        for u in users_list:
            subgraphs_mapping_i[0][u] = {}
            # for i in range(1, num_items):
            #     subgraphs_mapping_i[0][u][i] = i  # do not know if correct

    cumuindex = num_items
    cumuindex_record = {}
    for t in subgraphs_mapping_i:
        cumuindex_record[t] = cumuindex
        for u in subgraphs_mapping_i[t]:
            for i in subgraphs_mapping_i[t][u]:
                subgraphs_mapping_i[t][u][i] += cumuindex
            cumuindex += len(subgraphs_mapping_i[t][u])  # why


    ##### get the latest dynamic mapping the subgraph
    subgraphs_sequence_i = {} # do not know if correct, not differenciate u
    for i in range(1, num_items):
        subgraphs_sequence_i[i] = np.array([i] * (3 + len(sorted_time)))

    for t in subgraphs_mapping_i: # 因为以上sequence没有t&u，所以有的t没有更新，u也没有。
        for u in subgraphs_mapping_i[t]:
            for i in subgraphs_mapping_i[t][u]:
                subgraphs_sequence_i[i][t + 1:] = subgraphs_mapping_i[t][u][i]  # why # 前两个t空着。
    ...
    #     if t == 0:
    #         continue
    #     reversed_subgraphs_mapping_last_i[t] = {}
    #     for u in subgraphs_mapping_i[t]:
    #         reversed_subgraphs_mapping_last_i[t][u] = [0] * len(subgraphs_mapping_i[t][u])
    #         for i in subgraphs_mapping_i[t][u]:
    #             try:
    #                 reversed_subgraphs_mapping_last_i[t][u][subgraphs_mapping_i[t][u][i] - cumuindex_record[t]] = \
    #                 subgraphs_sequence_i[i][t]
    #             except IndexError:
    #                 print(f't:{t}\tu:{u}\ti:{i}') #2, 187, 1048
    #                 exit(0)

    return subgraphs_mapping_i, reversed_subgraphs_mapping_i, sorted_time, subgraphs_sequence_i
...
def create_user_side_hypergraphs(data):
    data_path = 'data/'
    ...
    all_t = [int(value) for key, values in t_map.items() for value in values]
    print(all_t)

    all_ui_interactions = {key: defaultdict(defaultdict) for key in all_t}
    ...
    for line in f:
        ...
        if u in all_ui_interactions[t_id]:
            all_ui_interactions[t_id][u].append(i)
        else:
            all_ui_interactions[t_id][u] = [i]

    # 找到similar users
    all_u_similar_uids = find_similar_users(all_t, users_set, all_ui_interactions)
    #
    # # save defaultdict to pickle
    pickle.dump(all_u_similar_uids, open(output_file, 'wb'))

    # 看看平均有多少个similar users
    # for t in all_u_similar_uids.keys():
    #     u_num = 0
    #     num = 0
    #     for u in all_u_similar_uids[t]:
    #         u_num += 1
    #         num += len(all_u_similar_uids[t][u])
    #
    #     print(f'time: {t}\t {num/u_num}')

    all_u_similar_uids = pickle.load(open(output_file,'rb'))
    # print(all_u_similar_uids)
    # existed_timestamp = [key for key in all_u_similar_uids if all_u_similar_uids[key] != all_u_similar_uids.default_factory()]
...
if __name__ == '__main__':
    ...
    subgraphs_mapping_i, reversed_subgraphs_mapping_i, sorted_time, subgraphs_sequence_i = subgraph_key_building(user_side_subgraphs_mapping_i, users_list, num_items)
    # user_side_subgraphs_mapping_u, user_side_reversed_subgraphs_mapping_u, user_side_sorted_time_u, user_side_subgraphs_sequence_u = subgraph_key_building(
    ...
    # pickle.dump(subgraphs_mapping_i, open(output_file_4, 'wb'))
    # pickle.dump(reversed_subgraphs_mapping_i, open(output_file_5, 'wb'))
    # pickle.dump(sorted_time, open(output_file_6, 'wb'))
    # pickle.dump(subgraphs_sequence_i, open(output_file_7, 'wb'))
    ...
    # pickle.dump(user_side_subgraphs_sequence_u, open(output_file_11, 'wb'))
