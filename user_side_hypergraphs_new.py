...
def subgraph_key_building(subgraphs_mapping_i, users_list, num_items):
    ...
    for t in subgraphs_mapping_i: # 因为以上sequence没有t&u，所以有的t没有更新，u也没有。
        for u in subgraphs_mapping_i[t]:
            for i in subgraphs_mapping_i[t][u]:
                try:
                    subgraphs_sequence_i[i][t + 1:] = subgraphs_mapping_i[t][u][i]  # why # 前两个t空着。#KEY ERROR: 18435
                except KeyError:
                    print(t)
                    print(u)
                    print(i)
...
def create_user_side_hypergraphs(data, interation, time, timestamp):
    ...
    # 找到similar users
    all_u_similar_uids = find_similar_users(list(timestamp), users_set, all_ui_interactions)
    #
