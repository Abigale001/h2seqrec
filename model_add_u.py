...
class NeuralSeqRecommender(object):
    def __init__(self, args, n_items, n_users, subgraphs_G, reversed_subgraphs_mapping_i, reversed_subgraphs_mapping_u, reversed_subgraphs_mapping_last_i,
                 sorted_time, subgraphs_G_group, reversed_subgraphs_mapping_i_group, reversed_subgraphs_mapping_u_group,user_side_sorted_time, n_hyper = 2):
        self.args = args
        ...
        for i in subgraphs_G_group:
            self.subgraphs_G_group[i] = {}
            for u in subgraphs_G_group[i]:
                self.subgraphs_G_group[i][u] = {}
                self.subgraphs_G_group[i][u]['G'] = _convert_sp_mat_to_sp_tensor(subgraphs_G_group[i][u]['G'])
                self.subgraphs_G_group[i][u]['E'] = _convert_sp_mat_to_sp_tensor(subgraphs_G_group[i][u]['E'])
    ...
    def _build(self):
        ...
        for i in self.sorted_time:
            ...
            x2 = tf.nn.embedding_lookup(emb_list, self.reversed_subgraphs_mapping_last_i[i]) # last

            stacked_features = tf.stack([x1,x2]) # static and dynamic item embeddings
            ...
            emb_list_user.append(edges) # users



            # # for u, learn similar user's i-s
            # for u in range(1, self.n_users): # do not know the exact user num
            #     # find similar user;
            #     # input: t; u; in time t, each user's items.
            #     # output: in time t, similar uids (or these u-s' items, split by diff u-s)
            #     exit(0)
        ...
        try:
            ...
            for t in self.user_side_sorted_time:
                if not t in self.subgraphs_G_group:
                    continue

                for u in tf.unstack(self.u_list, axis=0):
                    if u not in self.subgraphs_G_group[t]:
                        continue
                    input_items = tf.nn.embedding_lookup(self.item_embedding, self.reversed_subgraphs_mapping_i_group[t][u])
                    nodes, edges = HGNN_conv(input_dim=self.args.emsize,
                                             output_dim=self.args.emsize,
                                             adj=self.subgraphs_G_group[t][u],
                                             act=tf.nn.relu,
                                             dropout=self.dropout_graph,
                                             n_hyper=self.n_hyper)(input_items)

                    stacked_features_group = tf.stack([input_items, nodes])  # static and dynamic item embeddings
        ...
        outputs = self.net(input_item_all, mask) # 也可放到transformetNet里面。
